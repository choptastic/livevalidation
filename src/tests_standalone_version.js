/**
 * @author Alec Hill, Alexander Maslov
 * 
...
function runTests(){
  ...
  new Test.Unit.Runner({
    ...
    testAsyncCustomFunctionCall: function(){ with(this){
      // should return result of async function...
      assert(true, Validate.Custom(7,{
        against: function(value, args, callback) {
          setTimeout(function(){callback(window.isPrimeOrDivisibleBySpecified(value, args));},50);
        }
      }));
      try {
        assertNotEqual(true, Validate.Custom(6,{
          against: function(value, args, callback) {
            setTimeout(function(){callback(window.isPrimeOrDivisibleBySpecified(value, args));},50);
          }
        }));
